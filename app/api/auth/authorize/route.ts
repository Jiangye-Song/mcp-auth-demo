...
/**
 * OAuth 2.1    googleOAuthUrl.searchParams.set("client_id", process.env.GOOGLE_CLIENT_ID || "");Authorization Endpoint with MCP 2025-06-18 Compliance
...
export async function GET(request: NextRequest) {
    ...
    try {
      ...
      if (resourceUrl.origin !== new URL(baseUrl).origin) {
        ...
        return redirectWithError(
          redirectUri,
          "invalid_target",
          "Resource parameter must match server origin",
          state,
        );
      }
    } catch {
      ...
      return redirectWithError(
        redirectUri,
        "invalid_target",
        "Invalid resource parameter format",
        state,
      );
    }
  }
  ...
  if (!validRedirectUris.includes(redirectUri) && !isValidDynamicRedirect) {
    ...
    console.log(
      "📋 Allowed patterns: VS Code (127.0.0.1:port/), MCP Remote (localhost:port/oauth/callback)",
    );
    return NextResponse.json(
      {
        error: "invalid_request",
